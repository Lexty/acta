...
public enum Recovery {
    ...
    /// так что она проходит. Плюс требуем найти `fmt `/`data` в прочитанном префиксе: файл из
    /// `RIFF`/`WAVE` и нулей без осмысленных чанков `ffmpeg` отвергает, утаскивая за собой склейку
    /// всей дорожки; не нашли — подтвердить целостность нечем, сегмент отбрасывается.
    ///
