...
- [x] Unit test: the pure function building `ffmpeg` arguments is covered (`FFmpeg.concatArgs`/`concatListContents` + segment layout `SegmentLayout`). The mix builder was covered too until Task 12 removed the mix from the pipeline and `FFmpeg.mixArgs` with it
...
- [x] Update tests: delete `TrackSelection` tests and the "forces combined" normalisation tests; adapt `SegmentAssembler`/`RecordingSettings` tests. ⚠️ `FFmpeg.mixArgs` and its tests were kept at first, then removed in review — an argument builder for a file nothing produces reads as live code, and Export mix re-adds it against a real export path (see `docs/backlog/acta-full-plan.md`). Assertions may be **removed only where they test deleted behaviour**; they may not be broadened, skipped, or turned into "does not throw"
- [x] **Filesystem-level assembler tests** with fixture WAV segments (no capture needed — `SegmentAssembler` takes a directory, so this is fully automatable and is the only thing that proves the deletion did not break assembly):
  ...
  - `ffmpeg` concat fails → segment directories remain (the only copy of the audio is never destroyed). ⚠️ Note for whoever reads this next: a *broken segment* cannot produce this failure — `WAV.layout` refuses a `fmt ` that is not playable PCM, so such a segment drops out of the plan and the track just comes out shorter. The test uses two segments the plan accepts and the muxer will not splice under `-c copy` — the second declares a different sample rate and channel count — which fails `ffmpeg` at the concat step itself with a short, playable file already written (the shape that makes the failure dangerous). ⚠️ Blocking the **output** path with a directory was the earlier technique and no longer fails at all, now that the concat writes under a temp name. The dropped-segment path is worth proving too, and has its own test;
...
- [x] ⚠️ **Review follow-up — the mix was carrying the duration, and dropping it broke `info.md`.** `durationSeconds` picked the *first* readable track off `[combined, system, mic]`, and `combined` was always there and always mixed `duration=longest`. Removing it silently repointed the number at `system.wav`. The tracks are only nominally equal — a dropped segment shortens one alone — so an uneven pair now under-reported the meeting. Fixed with `.max()`; `assembleReportsTheLongerTrackWhenTheTracksAreUneven` pins it (reverted to `.first` it reports 1.5 s against a 2 s meeting)
- [x] ⚠️ **Review follow-up — `ffmpeg` exits 0 on a failed concat.** Measured: given a segment it cannot demux, `ffmpeg` logs the error, writes a **short** file and exits **0**. `runFFmpeg` only reads the exit status, so that read as a clean assembly — the meeting went `done` and, with `deleteSegments: true`, the segments holding the missing audio were deleted. Fixed with `-xerror` in `FFmpeg.concatArgs` (exits non-zero — the exact code depends on the failure; 183 and 234 both observed, so nothing reads it beyond `!= 0`). The plan drops unplayable segments before `ffmpeg`, so this needs a torn/vanished file to fire — but the failure mode it prevented is exactly the one this task's own tests are about: destroying the only copy
- [x] ⚠️ **Review follow-up — a partly-rescued track passed as a clean one.** The fix above kept the segments when a planned segment failed its repair, but `retainedSegments` never left `concatTrack`: `assemble` returned a `Result` indistinguishable from a clean assembly, so `stop()` wrote `done` and recovery wrote `recovered` — terminal, with a duration measured off the *truncated* track. The audio survived on disk with every automatic path that could rescue it closed, and the only trace was one `os_log` line. And this is the *likely* shape, not the exotic one: a crash leaves at most one unfinalized segment per track, so the whole-track loss that `segmentsUnrepairable` already caught is the rare case. Fixed by throwing `segmentsUnrepairable` on **any** retained segment — one exit for one condition, ahead of every deletion, reusing the "marker stays `recording`, retry next launch" plumbing that a failed assembly already uses
- [x] ⚠️ **Review follow-up — that retry needed a bound.** Widening `segmentsUnrepairable` widened an existing hole: `RecoveryManager` retried it forever. The enum's own premise ("reasons that commonly clear") does not survive inspection — `SegmentRepair.apply` overwrites eight bytes of an existing file, so it never needs a free block, and what actually stops it (read-only volume, wrong permission, immutable flag, failing hardware) is still there next launch. Unbounded, such a folder re-runs a full `ffmpeg` concat on every launch — and every `start()` awaits recovery — while reading "not finished" forever with no way to clear it, the exact fate the `noSegments` branch refuses to inflict. Fixed with `SessionManifest.assemblyAttempts` (absent in older markers → decodes to 0, hence the hand-written `init(from:)`): retry twice, then close with the tracks that did assemble and a duration measured off them. Giving up costs nothing — recovery never deletes, so the segments stay the audio's copy of record. Five tests in `RecoveryManagerRetryTests` drive it through a real read-only segment
- [x] ⚠️ **Review follow-up — the give-up's *partial* case was reported as a clean recovery.** The bound above told the user the truth only when it saved nothing: `retryOrCloseIncomplete` annotated `info.md` and reported `unassembled` for `tracks.isEmpty`, while the likelier shape — one track assembled, the other's audio still only in `system/` or `mic/` — went into `outcome.recovered` and produced "Recovered 1 interrupted recording", with an `info.md` reading `status: recovered` and a duration measured off the surviving track. Every visible signal said whole; half the meeting was missing and no launch would ever retry it (the marker is terminal). The playable wav is what makes it invisible — with nothing to open, a user at least *looks*. Fixed by splitting the give-up in two: `Outcome.partial` alongside `recovered`/`unassembled`, `RecoveryReport.message(recovered:partial:unassembled:)` with its own sentence and a "Recordings partially recovered" title, and `RecoveryManager.partialNote` in `info.md` saying where the rest is and that `duration` is the track's length, not the meeting's
...
- [x] Acceptance (automatable): identifier grep — `grep -rn "TrackSelection\|saveCombinedTrack\|saveSystemTrack\|saveMicTrack" Sources/Acta Sources/ActaRuntime Sources/ActaKit` returns nothing. ⚠️ Scoped to the product targets, not all of `Sources/`, and the reason is the checkbox two above: the decode-only migration test **must** carry the old keys as literal JSON (`{"saveSystemTrack":true,...}`) — that string is the entire point of the test — expected there, nowhere in the product code
- [x] Acceptance (automatable): **behaviour grep** — `grep -rn "combined\.wav\|combinedWAV\|mixFailed" Sources/Acta Sources/ActaRuntime Sources/ActaKit` finds no runtime assembly branch. ⚠️ The identifier grep alone cannot prove this: a mix branch can survive under different names. `FFmpeg.mixArgs` went with the mix (see the checkbox above), so no mix argument builder is expected in `ActaKit` or in the tests either
- [x] Acceptance (automatable): `swift build -c release`, `bash Scripts/test.sh` (186 tests), `bash Scripts/lint.sh`, `bash Scripts/bundle.sh dev` all green
- [ ] Morning check (needs a human, **not** a blocking checkbox): a dev recording produces exactly two final audio files and no `combined.wav` — **still open, and it cannot be closed from here**. The filesystem-level assembler tests make the same claim from fixture segments, which is the closest an automated test gets, but they never touch live capture — the one thing this check exists to exercise. Marking it `[x]` on the strength of the fixture tests was claiming the evidence the tests cannot supply
