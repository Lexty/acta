...
**All tasks are done.** Tasks 1–9 are done and verified by a live run. Task 10 (display sleep) jumped
the queue because a live run on 2026-07-15 proved the app unusable for real meetings without it. Task
11 was the extraction everything else waited on; with `ActaRuntime` importable, the backlog's test
infrastructure is now buildable.
...
- [x] Acceptance (automatable): while a recording runs, `pmset -g assertions` lists the assertion held by Acta with the expected reason; after stop it is gone; after a failed start it is gone (`DisplayWakeLockTests` — 6 tests that shell out to `/usr/bin/pmset` rather than trusting `isHeld`: acquired → the OS lists the reason; released → it is gone; dropped without release → gone. `.serialized`, because an assertion is process-global and swift-testing parallelizes by default. Not vacuous: the positive expectation would fail if `pmset` returned nothing. The *failed-start* release is covered at the `DisplayWakeLock` level, not through `RecordingSession` — that type is unreachable from tests until Task 11 makes it importable)
...
- [x] Access control: whatever must cross the target boundary becomes `public` — **mechanically**. Do not take the opportunity to redesign, narrow or "clean up" the API surface. A `public` keyword is the whole change (verified by diffing the renames: **every** changed line in the moved files is a `public` keyword, the public memberwise `init` that `MeetingStore.Recording` now needs — a public struct's memberwise init is internal, so `ActaApp` could not construct one without it — or a continuation-indent realignment forced by the wider `public func` signature, which SwiftLint's `vertical_parameter_alignment` requires. No type, signature, name or behaviour changed)
