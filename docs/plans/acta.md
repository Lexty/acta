...
- [x] Юнит-тесты: детектор «данные не текут» на фейковом источнике; выбор действия по типу ошибки (`DiagnosticsTests`: `isDataFlowing`/`diagnose` по снимкам, `action` по типу и остатку попыток, `FlowWatchdog` на синтетических последовательностях счётчика — 18 тестов)
