...
- [x] **New `stop()` ordering** (exact sequence):
  ...
  6. keep the stream object alive through steps 3–4. **Implemented differently than written here, deliberately:** the slot is cleared *first*, in the same lock acquisition that takes the stream (`takeStreamsForTeardown()`), and the returned array is what retains it across the awaits. Clearing "last" cannot be done under that one lock, so a restart storing a new stream mid-teardown would have had its stream cleared out from under it. The aliveness rationale is preserved; only the mechanism differs
...
- [x] ⚠️ **Behaviour must not change except the teardown.** Buffer delivery, the start path, the `.streamNotStarted` mapping, the stream-identity check in `didStopWithError`, and the per-track queues all stay exactly as they are. No `removeStreamOutput` is added. `isStreaming` reads `false` for the teardown's duration (the slot is cleared under the take's lock — see step 6) — acceptable because `AudioRecorder` serializes stop against start/restart/self-check, and `RecordingSession.stop()` cancels and awaits the watchdog before `recorder.stop()`, so no self-check can observe that window
...
- [x] Acceptance (review — this is what verifies the production ordering, since no test does): a reviewer confirms the exact sequence — gate closed first; the no-stream path still drains; `updateConfiguration(mic off)` awaited before `stopCapture()`; separate `do`/`catch` per call with each failure logged; all three queues drained; the stream retained across both awaits (via the array `takeStreamsForTeardown()` returns — see step 6) and the **live** stream torn down before any set-aside one; and the failed-start cleanup still throws `.streamNotStarted`
