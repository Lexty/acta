...
- [ ] `ExportMix` in `Acta`: `ffmpeg` `amix=inputs=2:duration=longest` over `system.wav`/`mic.wav` → `combined.wav`. Re-add the argument builder (`FFmpeg.mixArgs` was removed — the `amix=inputs=2:duration=longest:normalize=0` recipe is recorded in the note above). Write to a **temp file and rename atomically** — a failed or killed `ffmpeg` must never leave a plausible-looking `combined.wav`. `ffmpeg` missing → the existing actionable error; a track missing → clear message; an existing `combined.wav` → replaced only on success. Off the main actor via the `ProcessRunner` seam
...
- [ ] Tests for the export itself: the temp-file/rename commit, the guards, and a failed `ffmpeg` leaving no `combined.wav` — plus the re-added mix argument builder, which arrives with this task and carries no coverage from Task 12
...
**Plan 2 — MOUNTED** (`docs/plans/2026-07-17-socket-transport.md`, Codex-approved over two rounds): the POSIX Unix-socket transport hosted in the app, depending on the `ControlRequestHandling` abstraction Plan 1 exposes (not the singleton). It also settles the `settingsSet` archive-relocation authority Plan 1 deferred (a socket-specific dispatcher ignores a wire archive path). Security specifics baked in:
...
- [ ] Update tests: delete `TrackSelection` tests; adapt `SegmentAssembler`/`RecordingSettings` tests; cover the mix argument builder if this task re-adds one
