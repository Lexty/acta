# Backlog: full Acta plan (NOT for execution)

> This is a **backup**, not a plan to run. It is deliberately kept outside `docs/plans/` so that
> ralphex cannot pick it up.
>
> Saved on 2026-07-15, when the plan was cut back to its core (Tasks 10–13 — simplify, test seam,
> lifecycle, durability). Two rounds of external review had found progressively **more** defects, not
> fewer — the second round found ~15, several of which were introduced while fixing the first round's
> findings. That is the signature of a plan that has outgrown what can be held in one head, and the
> app it plans for already works and is verified.
>
> Tasks 14–20 below (sleep, device changes, menu-bar timer, auto-stop, calendar, mic-activity prompt,
> silence trimming) are parked here **as written at the time of the cut**. They still contain known
> defects flagged by the second review round and must be re-fixed before any of them is promoted back
> into `docs/plans/acta.md`. Known outstanding issues at the time of parking:
>
> - Task 13/14 contradiction: "pad or mark gaps" vs "do not pad the sleep gap".
> - Calendar re-match at stop cannot derive the wall-clock end from `started_at` + audio duration,
>   because sleep gaps are deliberately omitted from the audio.
> - Auto-stop: countdown duration lost in an edit; cancellation exists only as a notification action,
>   so it disappears when notifications are denied; a single calibrated threshold is unstable across
>   input devices; RMS/window/dBFS semantics undefined.
> - Calendar ranking is not a total order; title provenance (empty vs auto-suggested vs user-edited)
>   is not tracked.
> - Task 18b sits outside the numeric execution model ralphex follows.
> - Silence trimming does not say how onset/offset are detected, and needs a paired atomic commit.
>
> Promote items from here one at a time, only after the core has landed and been verified live.
